...
class DualDiceBCELoss(nn.Module):
    def __init__(self):
        super(DualDiceBCELoss, self).__init__()
        self.dicebce = DiceBCELoss()
        self.mse = nn.MSELoss()

    def forward(self, outputs, targets):
        if isinstance(outputs, tuple) and len(outputs) == 2:
            # 双网络模式: outputs = ((out1_1, out1_2, out1_3), (out2_1, out2_2, out2_3))
            (out1_1, out1_2, out1_3), (out2_1, out2_2, out2_3) = outputs
            
            # 计算每个网络的loss (加权，out1结尾的最重要)
            loss1 = 0.6 * self.dicebce(out1_1, targets) + 0.3 * self.dicebce(out1_2, targets) + 0.1 * self.dicebce(out1_3, targets)
            loss2 = 0.6 * self.dicebce(out2_1, targets) + 0.3 * self.dicebce(out2_2, targets) + 0.1 * self.dicebce(out2_3, targets)
            
            # 计算一致性损失 (同样加权)
            consistency_loss = 0.6 * self.mse(out1_1, out2_1) + 0.3 * self.mse(out1_2, out2_2) + 0.1 * self.mse(out1_3, out2_3)
            
            # 0.45 * loss1 + 0.45 * loss2 + 0.1 * consistency
            total_loss = 0.45 * loss1 + 0.45 * loss2 + 0.1 * consistency_loss
            return total_loss
        else:
            # 单网络模式: outputs = (out_1, out_2, out_3) (同样加权)
            out_1, out_2, out_3 = outputs
            loss = 0.6 * self.dicebce(out_1, targets) + 0.3 * self.dicebce(out_2, targets) + 0.1 * self.dicebce(out_3, targets)
            return loss


class DualUNetLoss(nn.Module):
    """针对双UNet网络的损失函数"""
    def __init__(self):
        super(DualUNetLoss, self).__init__()
        self.dicebce = DiceBCELoss()
        self.mse = nn.MSELoss()
    
    def forward(self, outputs, targets):
        if isinstance(outputs, tuple) and len(outputs) == 2:
            # 双网络模式: outputs = (net1_out, net2_out)
            net1_out, net2_out = outputs
            
            # 计算每个网络的分割损失
            loss1 = self.dicebce(net1_out, targets)
            loss2 = self.dicebce(net2_out, targets)
            
            # 计算一致性损失（两个网络输出应该相似）
            consistency_loss = self.mse(net1_out, net2_out)
            
            # 组合损失：0.4 * loss1 + 0.4 * loss2 + 0.2 * consistency
            total_loss = 0.4 * loss1 + 0.4 * loss2 + 0.2 * consistency_loss
            return total_loss
        else:
            # 单网络模式回退
            return self.dicebce(outputs, targets)