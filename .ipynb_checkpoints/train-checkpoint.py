...
class Trainer_seg:
    ...
    def _is_dual_network_output(self, output):
        """检查是否为双网络输出格式: ((out1_1, out1_2, out1_3), (out2_1, out2_2, out2_3))"""
        return (isinstance(output, tuple) and len(output) == 2 and 
                isinstance(output[0], tuple) and isinstance(output[1], tuple))

    def _get_evaluation_output(self, output):
        """从不同格式的输出中提取用于评估的张量"""
        if self._is_dual_network_output(output):
            # 双网络模式: 使用第一个网络的第一个输出
            (out1_1, out1_2, out1_3), (out2_1, out2_2, out2_3) = output
            return out1_1
    ...
    def _train(self, epoch):
        ...
        for batch_idx, (x_in, target) in enumerate(self.loader_train.Loader):
            ...
            # 获取用于评估的输出
            eval_output = self._get_evaluation_output(output)
            output_argmax = torch.where(eval_output > 0.5, 1, 0).cpu()
